...
Install it using:
<PackageCmd packages={['@graphql-inspector/ci']} />

You need to install `graphql` globally too:
<PackageCmd packages={['graphql']} />
...
<PackageCmd packages={['@graphql-inspector/diff-command']} />

You need to be able to load `schema.graphql` from disk and from `origin/master` on Git:

<PackageCmd global packages={['@graphql-inspector/graphql-loader @graphql-inspector/git-loader']} />
...
You can use GraphQL Inspector Docker images in any CI/CD provider. For example, in CircleCI it would look like this:
