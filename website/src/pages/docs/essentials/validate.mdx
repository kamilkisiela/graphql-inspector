...
- `--keepClientFields <b>` - Keeps the fields with `@client`, but removes `@client` directive from them - works only
  with combination of `--apollo` (default: `false`)
...
When there's at least one error or a deprecated usage (when `--deprecated` flag is enabled), the process fails.
