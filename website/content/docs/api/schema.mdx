...
const typeDefs = loadFilesSync('\*_/_.graphql')
...
// as `schema` variables export const schema = makeExecutableSchema({ typeDefs })
...
<Callout type="warning">GitHub Loader requires a GitHub token to be defined!</Callout>

## Programmatic API

If you are using programmatic API, you might find `@graphql-tools/load` package useful for loading
schemas. Learn more [here](https://graphql-tools.com/docs/schema-loading).
